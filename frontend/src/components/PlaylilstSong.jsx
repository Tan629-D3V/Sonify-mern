...
const PlaylilstSong = ({ title, artistName, songSrc ,playlistId }) => {
  ...
  const removeSong = async () => {
    try {
      console.log(`Removing song "${title}" from playlist ${playlistId}`);
      ...
      alert('Error removing song from playlist: ' + (error.response?.data?.message || error.message || 'Unknown error'));
    }
  };
